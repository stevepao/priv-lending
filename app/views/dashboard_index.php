...
<p class="text-slate-600">Below is a quick bank-level snapshot: loans by funding source, then the last three calendar months of cash activity attributed to each bank via <strong>Deposit to</strong> on cash events (current month is through today). Balances match the Loans list (principal in + principal out).</p>
</div>
</section>
...
<tbody>
<?php if ($jpmLoans === []): ?>
<tr><td class="py-3 text-slate-500" colspan="3">No loans with funding JPM.</td></tr>
<?php else: ?>
...
<tbody>
<?php if ($ntrsLoans === []): ?>
<tr><td class="py-3 text-slate-500" colspan="3">No loans with funding NTRS.</td></tr>
<?php else: ?>
