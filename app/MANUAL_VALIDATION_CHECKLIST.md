...
28. **GET /payoff/pdf** — Logged-in only (same gate as other app routes). Query **`loan_id`**, **`date_quoted`** (`Y-m-d`), **`payoff_good_thru`** (`Y-m-d`), optional **`last_month_interest_paid=1`**; invalid or **`payoff_good_thru`** &lt; **`date_quoted`** redirects **GET /payoff?invalid=1**. Recomputes payoff on the server (same path as **POST /payoff**), renders **`payoff_statement_pdf`** for Dompdf, streams **Content-Disposition** attachment **`Loan Payoff Statement - &lt;sanitized loan name&gt; - &lt;date_quoted&gt;.pdf`**. Requires **`composer install`** so **`dompdf/dompdf`** is present. HTML statement shows a centered **Download PDF** button (same params, including the last-month-paid flag).
...
- **Last month already paid** — With the form checkbox checked, the prior full-cycle **Interest -** line is absent and **Total** = principal + per-diem only; unchecked restores the full-month line. PDF download preserves the same flag.
